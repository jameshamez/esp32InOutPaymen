...
- `src/main.cpp` — the entire Arduino firmware in one file (~1700 lines), all in an anonymous namespace plus `setup()`/`loop()`. Structure to know:
  ...
  - **Payment confirmation / GPIO pulse**: `confirmPayment()` dedupes by `paymentId` and triggers `startPaymentPulsesForAmount()`, which drives a non-blocking pulse-queue state machine (`processPaymentPulses()`, called every `loop()`). Two modes, selected via `twoPinPulseMode` (`/api/pulse/config?mode=1pin|2pin`): 1-pin mode pulses GPIO 15 once per whole THB; 2-pin mode splits the amount into tens (GPIO 2) and ones (GPIO 15) digit pulses. Total pulses capped at `MAX_PAYMENT_PULSES`.
  - **P-Points integration**: two parallel flows against `https://p-points.com/sms_payin_rd.php` (`fetchPpointsResult` → `parsePpointsResponse`). `amt` reflects the latest single SMS record (not a running cumulative total — confirmed by observation, it can go down between checks), so both flows trigger on the record **count** changing (`lastPpointsCount`), not on the amount increasing, and pulse using that record's own `amt` value rather than a delta. The QR-session flow (`processPpointsResult`) records a baseline count/total, then a 5-minute session (`ppointsSessionActive`/`ppointsSessionExpiresAt`, `PPOINTS_QR_TTL_MS`) during which the display holds the QR while the frontend polls `/api/ppoints/check` every `PPOINTS_POLL_INTERVAL_MS`; the session expires without pulsing if no new record is seen in time. Separately, `processPpointsMonitor()` runs an always-on 5-second auto-poll (`/api/ppoints/monitor?action=start|stop`, `ppointsMonitorEnabled`/`ppointsMonitorIntervalMs`) that pulses on any new record detected without requiring a QR session at all. Both flows call `confirmPayment` and mirror what the Node dashboard does for local-only testing (the dashboard's `local-dashboard/server.js` still uses the older amount-delta comparison — check whether it needs the same count-based fix if it's relied on for real testing).
...
- The firmware has two physical GPIO outputs for payment confirmation pulses (GPIO 15 = ones/whole-THB, GPIO 2 = tens digit in 2-pin mode), selected via `twoPinPulseMode`; total pulse count capped at `MAX_PAYMENT_PULSES` = 200, driven non-blocking from `loop()`.
