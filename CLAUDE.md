...
PaymentESP generates Thai PromptPay QR codes on real ESP32 DevKit V1 hardware with a TJC4832T135 touchscreen display. It supports static QR, dynamic QR with amount, a web setup page, serial output, REST POST webhooks, and device logs. A Node.js "local dashboard" runs on a computer on the same LAN to configure the device and optionally receive webhook events and process Omise PromptPay charges. README.md is in Thai and covers operational behavior (setup flow, P-Points, API list) — consult it for details beyond what's summarized here. **Caveat:** README.md's hardware wiring section still describes the original OLED SSD1306 (`SDA->GPIO 21, SCL->GPIO 22`) and has not been updated since the `0203c6d` display swap to the TJC4832T135 — for actual display wiring/protocol, trust `src/main.cpp` (`Serial2`, GPIO16 RX2 / GPIO17 TX2) over the README. The README's Wokwi section also still says the payment pulse fires on "GPIO 26" — the real pin is GPIO 15 (`PAYMENT_PULSE_PIN` in `src/main.cpp`, GPIO 2 in 2-pin mode).
...
- `src/main.cpp` — the entire Arduino firmware in one file (~1700 lines), all in an anonymous namespace plus `setup()`/`loop()`. Structure to know:
  ...
  - **WiFi bootstrap**: `connectWifi()` tries the saved STA credentials; on failure it falls back to a `PaymentESP-Setup` AP (open at `192.168.4.1/setup`) served by the same `WebServer` instance, gated by `setupMode`. The `/setup` form covers WiFi SSID/password, the mDNS device hostname (`mdnsHostname`, persisted, replaces the old hardcoded `paymentesp.local`), and the webhook URL — it does **not** include PromptPay ID (removed from both `/setup` and the main page's UI; still settable via `POST /api/config` and defaults to NVS/`DEFAULT_PROMPTPAY_ID`).
  ...
  - **Display rendering**: `render*()` functions (`renderEventToOled`, `renderPaymentConfirmed`, `renderPpointsDelta`, `renderCustomerQrStatus`, `renderNetworkStatus`, etc.) send Nextion/TJC protocol UART commands (`include/DisplayHMI.h`/`src/DisplayHMI.cpp`, host-testable like `PromptPayQR.h`/`.cpp`) to a TJC4832T135 touchscreen over `Serial2` (GPIO16 RX2 / GPIO17 TX2, 115200 baud). Status text goes to four Text components (`t0`-`t3`); the QR itself is drawn as filled-rectangle blocks directly on the page background (`sendQrToDisplay`/`clearQrArea` in `main.cpp`) rather than through a native QR component, because TJC's T1-series QRcode component hard-caps text input at 84 bytes — shorter than this firmware's real PromptPay payloads. Both `t0`-`t3` and the reserved QR drawing area (x=320,y=160,150x150 — bottom-right of the 480x320 screen) must exist in the `.tft` project uploaded to the display through TJC's USART HMI Editor. `qrcode.h`/`buildQrCode()` is shared with `qrSvgFromText()` (the web UI's `/api/qr.svg`).
  - **Payment confirmation / GPIO pulse**: `confirmPayment()` dedupes by `paymentId` and triggers `startPaymentPulsesForAmount()`, which drives a non-blocking pulse-queue state machine (`processPaymentPulses()`, called every `loop()`). Two modes, selected via `twoPinPulseMode` (`/api/pulse/config?mode=1pin|2pin`): 1-pin mode divides the whole-THB amount by `pulseDivisorOnes` and pulses GPIO 15 that many times; 2-pin mode first divides by `pulseDivisorTens` for GPIO 2 (the remainder then divides by `pulseDivisorOnes` for GPIO 15) — e.g. divisor1=1/divisor2=10 (the defaults) reproduces the classic ones/tens digit split. Divisors, pulse width, and inter-pulse gap are runtime-configurable via `/api/pulse/config?divisor1=&divisor2=&width=&gap=` (ranges 0-100, 0-10000, 1-200ms, 1-200ms; defaults 1, 10, 25ms, 25ms) — matches an external coin-pulse-counter spec (O/P#1, O/P#2, PulseWidth, PulseSpaceWidth). None of these are persisted to NVS, so they reset to defaults on reboot. Total pulses per output capped at `MAX_PAYMENT_PULSES`.
  - **P-Points integration**: two parallel flows against `https://p-points.com/sms_payin_rd.php` (`fetchPpointsResult` → `parsePpointsResponse`). `amt` reflects the latest single SMS record (not a running cumulative total — confirmed by observation, it can go down between checks), so both flows trigger on the record **count** changing (`lastPpointsCount`), not on the amount increasing, and pulse using that record's own `amt` value rather than a delta. The QR-session flow (`processPpointsResult`) records a baseline count/total, then a 5-minute session (`ppointsSessionActive`/`ppointsSessionExpiresAt`, `PPOINTS_QR_TTL_MS`) during which the display holds the QR while the frontend polls `/api/ppoints/check` every `PPOINTS_POLL_INTERVAL_MS`; the session expires without pulsing if no new record is seen in time. Separately, `processPpointsMonitor()` runs a configurable auto-poll (`/api/ppoints/monitor?action=start|stop`, `ppointsMonitorEnabled`/`ppointsMonitorIntervalMs`) that pulses on any new record detected without requiring a QR session at all. Polling mode (`polling_mode=continuous|session`, `ppointsMonitorContinuous`) picks between running forever until an explicit `action=stop` (1A, the default) or auto-stopping itself after `timeout_ms` (`ppointsMonitorTimeoutMs`, default 5 min) elapses since `action=start` (1B) — checked at the top of `processPpointsMonitor()` before the per-check interval gate. None of `ppointsMonitorContinuous`/`ppointsMonitorIntervalMs`/`ppointsMonitorTimeoutMs` persist to NVS; they reset to defaults on reboot. Both flows call `confirmPayment` and mirror what the Node dashboard does for local-only testing (the dashboard's `local-dashboard/server.js` still uses the older amount-delta comparison — check whether it needs the same count-based fix if it's relied on for real testing).
...
- `deliverables/` (gitignored, generated by the `scripts/build_*_docx.py` scripts — not present until you run them) contains prebuilt, versioned delivery packages (source + firmware + documents + evidence) — treat as generated/archival output, not a place to make source edits.
...
- The firmware has two physical GPIO outputs for payment confirmation pulses (GPIO 15 = O/P#1, GPIO 2 = O/P#2, used only in 2-pin mode), selected via `twoPinPulseMode`; per-output divisor, pulse width, and gap are runtime-configurable (see P-Points/GPIO pulse section above) but not persisted to NVS. Total pulse count capped at `MAX_PAYMENT_PULSES` = 200, driven non-blocking from `loop()`.
